...
async function setMeta(meta: WindowsMetaData) {
    ...
    .replace("© Deno contributors & Deno Land Inc. MIT licensed.", meta.copyright)
    .replace('ProductName = "Deno"', `ProductName = "${meta.productName}"`)
    .replace('FileDescription = "Deno: A secure runtime for JavaScript and TypeScript"', `FileDescription = "${meta.description}"`);
...
async function compileSource() {
    ...
    const spinner = wait("Building Deno from source. This can take a few minutes.").start();
    await p.status();
    spinner.succeed();
    spinner.stop();
}
...
async function moveBuild(name: string = "deno") {
    await Deno.rename(RELEASE_BINARY, name);
}

await initaliseSource();
await embed("console.log(1)");
await compileSource();
await moveBuild("myBinary")